...
void define_first_coord(jgl::Data param)
{
	...
	engine->editor_interacter()->pink_flag() = screen_to_tile(g_mouse->pos);
}

void define_second_coord(jgl::Data param)
{
	engine->editor_interacter()->blue_flag() = screen_to_tile(g_mouse->pos);
}
...
void define_selected_npc(jgl::Data param)
{
	...
	engine->editor_interacter()->selected_entity() = tmp;
}

void add_selected_npc_check_point(jgl::Data param)
{
	Entity* select_npc = engine->editor_interacter()->selected_entity();
